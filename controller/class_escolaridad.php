...
class escolaridad{
	...
	*/
	function seleccionaEscolaridadCmb(){
		$sql = "select * from insotic_tabla where tipo='ESC' order by nombre asc";
		$this->conectar();
		$r = mysql_query($sql);
		if (mysql_num_rows($r)==0) { 
			...
			while ($f = mysql_fetch_object($r)){
				echo "<option value='$f->id'>$f->valor</option>";
            }
			echo "</select>";	
		}		
	}
	...
	*/
	function updateEscolaridad($id, $a){
		$sql = "update insotic_tabla set valor = '$a[0]' where id = '$id'";
		$this->conectar();
		if(mysql_query ($sql)){
			return true;
		} 
